...
read_deg <- function(file) {
  ...
  if (dup_fraction > 0.15) {
    warning(
      "High duplicate rate: ", round(dup_fraction * 100, 2),
  ...
  if (final_rows != original_rows) {
    warning(
      "The number of rows has changed from ", original_rows, " to ", final_rows,
  ...
  if (nrow(significant_df) < 100) {
    warning(
      "After filtering for pvalue <= 0.05, the number of rows is ", nrow(significant_df),
  ...
  if (prop_extreme > 0.10) {
    warning(
      "More than 10% of log2FoldChange values exceed an absolute value of ", extreme_threshold,

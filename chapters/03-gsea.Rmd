...
gsea_paver_input <- purrr::map_dfr(global_state$data, function(x) {
    dplyr::bind_rows(x$bpn@gsea@sig_pos_enriched, x$bpn@gsea@sig_neg_enriched)
}, .id = "Group") %>%
...
GSEA_PAVER_result <- PAVER::prepare_data(
    gsea_paver_input,
...
list(GSEA_PAVER_result, "NES") %>%
    knitr::knit_child(
        text = readLines(here::here("Rmd", "paver_report.Rmd")),
        envir = environment(),
        quiet = TRUE
    ) %>%
    cat(sep = "\n")
